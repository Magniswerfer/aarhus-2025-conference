...
export default function MobileMenu(
  ...
  const isDropdownActive = (link: NavigationItem) => {
    ...
    if (link.dropdownItems?.some(item => item.path === currentPath)) return true;
    return false;
  };
                ...
                <div class="flex items-center">
                  ...
                  {link.hasDropdown && (
                    <button 
                      ...
                      class="px-4 py-2"
                      aria-label={expandedDropdowns[link.path] ? "Collapse" : "Expand"}
                    >
                      <svg 
                        class={`w-5 h-5 transition-transform duration-200 ${expandedDropdowns[link.path] ? 'rotate-180' : ''}`} 
                        fill="none" 
                        stroke="currentColor" 
                        viewBox="0 0 24 24"
                      >
