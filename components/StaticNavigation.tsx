...
export default function StaticNavigation({ items, currentPath }: StaticNavigationProps) {
  const staticLinks: NavigationItem[] = [
    ...
    { path: "/past-conferences", label: "PAST CONFERENCES", hasDropdown: false, order: 5 },
    { path: "/organisers", label: "ORGANISERS", hasDropdown: false, order: 6 },
  ];
  ...
  return (
    <nav class="relative bg-transparent px-4 md:px-12 py-2">
      <div class="flex items-center justify-between">
        ...
        <MobileMenu navigationItems={sortedNavigationItems} currentPath={currentPath} />
      </div>
    </nav>
  );
}
